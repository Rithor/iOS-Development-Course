//
//  ViewController.m
//  25.01.UIButton.ExercisePupil
//
//  Created by Vitaliy on 12/04/2019.
...
typedef enum {
    ...
    CalculatorButtonPercent         = 12,
    CalculatorButtonDivision        = 13,
    CalculatorButtonMultiplication  = 14,
...
- (void)setMathOperationWithTag:(UIButton *)sender withEqualOperation:(BOOL)needEqualOperation {
    ...
    switch (sender.tag) {
        ...
        case CalculatorButtonMultiplication:
            if (needEqualOperation) {
                [self actionEqual:sender];
            }
            self.operationDisplayLabel.text = [NSString stringWithFormat:@"☓"];
        ...
        case CalculatorButtonPercent:
            ...
            self.operationDisplayLabel.text = [NSString stringWithFormat:@"％"];
            self.lastMathOperation = LastMathOperationPercent;
            break;
...
- (IBAction)typeNumberButton:(UIButton *)sender {
    ...
    //проверяем: начинаем ли мы вводить новое число. С помощью этой проверки обходим ошибку, когда у второго числа не нажимается ноль
    if ([self.typeNumber length] == 0) {
...
- (IBAction)actionArithmeticOperations:(UIButton *)sender {
    ...
    //ставим метку, что было нажатие на кнопку с математической операцией, следовательно необходимо запомнить число и быть готовым к набору нового. Это действие реализуется в другом методе - (IBAction)typeNumberButton:(UIButton *)sender {, что позволяет нам выбирать математическое действие несколько раз
